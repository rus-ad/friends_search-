        ...
        for country in Countries['items']:
            ...
            for city in city_list['items']:
                ...
                for row in df.index:
                    ...
                    time.sleep(0.35)
                    emb_girl = img_requests(df.loc[row]['photo_max_orig'])
                    if len(emb_men) == 128 and len(emb_girl) == 128:
                        asyncio.run(main(id_val=df.loc[row]['relation_partner']['id'],
                                         photo_girls=emb_girl,
                                         photo_mens=emb_men,
                                         id_girls=girl,
                                         id_men=men,
                                         ))
                        ...
                        print(len(emb_men), len(emb_girl))
                    count_requests += 1
